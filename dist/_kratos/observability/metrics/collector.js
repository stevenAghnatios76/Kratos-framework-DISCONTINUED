...
class MetricsCollector {
    ...
    record(metric) {
        ...
        db.run(`INSERT INTO metrics (metric_type, metric_name, value, unit, dimensions, recorded_at)
       VALUES (?, ?, ?, ?, ?, datetime('now'))`, [
            metric.metric_type,
            metric.metric_name,
            metric.value,
    ...
    query(opts) {
        ...
        if (!result.length)
            return [];
        return result[0].values.map((row) => this.rowToMetric(result[0].columns, row));
    }
    /**
     * Aggregate a metric over a period.
     */
    aggregate(metric_type, metric_name, fn, dateFilter) {
        const db = this.manager.getDatabase();
    ...
    prune(olderThanDays) {
        const db = this.manager.getDatabase();
        const countResult = db.exec(`SELECT COUNT(*) FROM metrics WHERE recorded_at < datetime('now', '-${olderThanDays} days')`);
    ...
    rowToMetric(columns, row) {
        ...
        return {
            ...
            value: obj.value,
            unit: obj.unit,
            dimensions: JSON.parse(obj.dimensions || '{}'),
