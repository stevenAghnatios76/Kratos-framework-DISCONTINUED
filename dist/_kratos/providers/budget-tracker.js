...
class BudgetTracker {
     ...
     */
    async recordSpend(record) {
        const db = this.manager.getDatabase();
        db.run(`INSERT INTO metrics (metric_type, metric_name, metric_value, dimensions, recorded_at)
    ...
    async getDailySpend() {
        const db = this.manager.getDatabase();
        const result = db.exec(`SELECT COALESCE(SUM(metric_value), 0) FROM metrics
     ...
     */
    async getSpend(period) {
        const db = this.manager.getDatabase();
        const dateFilter = this.periodToDateFilter(period);
        // Total cost
        const totalResult = db.exec(`SELECT COALESCE(SUM(metric_value), 0), COUNT(*) FROM metrics
       WHERE metric_type = 'cost' ${dateFilter}`);
        ...
        // By provider
        const byProviderResult = db.exec(`SELECT json_extract(dimensions, '$.provider'), SUM(metric_value) FROM metrics
       WHERE metric_type = 'cost' ${dateFilter}
       GROUP BY json_extract(dimensions, '$.provider')`);
        ...
        // By workflow
        const byWorkflowResult = db.exec(`SELECT json_extract(dimensions, '$.workflow'), SUM(metric_value) FROM metrics
       WHERE metric_type = 'cost' ${dateFilter}
       GROUP BY json_extract(dimensions, '$.workflow')`);
        const by_workflow = {};
        if (byWorkflowResult.length) {
            for (const row of byWorkflowResult[0].values) {
                by_workflow[row[0]] = row[1];
            }
        }
        // By model (as tier proxy)
        const byModelResult = db.exec(`SELECT json_extract(dimensions, '$.model'), SUM(metric_value) FROM metrics
       WHERE metric_type = 'cost' ${dateFilter}
     ...
     */
    async calculateSavings(period) {
        const db = this.manager.getDatabase();
        const dateFilter = this.periodToDateFilter(period);
        // Actual cost
        const actualResult = db.exec(`SELECT COALESCE(SUM(metric_value), 0) FROM metrics
       WHERE metric_type = 'cost' ${dateFilter}`);
        const actual_cost = actualResult[0]?.values[0]?.[0] || 0;
        // Calculate Opus baseline from token counts
        const tokensResult = db.exec(`SELECT
       ...
       FROM metrics WHERE metric_type = 'cost' ${dateFilter}`);
        const totalInput = tokensResult[0]?.values[0]?.[0] || 0;
        const totalOutput = tokensResult[0]?.values[0]?.[1] || 0;
