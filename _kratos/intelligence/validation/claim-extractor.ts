...
export class ClaimExtractor {
  ...
  extract(content: string): ExtractedClaim[] {
    ...
    for (const [type, pattern] of Object.entries(ClaimExtractor.PATTERNS)) {
      ...
      while ((match = regex.exec(content)) !== null) {
        ...
        if (type === 'file_ref' && this.isLikelyNotFilePath(value)) continue;
        if (type === 'version' && this.isLikelyNotVersion(value)) continue;
        if (type === 'dependency' && this.isLikelyNotDependency(value)) continue;
  ...
  private isLikelyNotFilePath(value: string): boolean {
    // Skip URLs
  ...
  private isLikelyNotVersion(value: string): boolean {
    // Skip standalone small numbers like "1.0" without context
    if (/^\d+\.\d+$/.test(value) && parseFloat(value) < 1) return true;
    return false;
  }

  private isLikelyNotDependency(value: string): boolean {
  ...
  private deduplicate(claims: ExtractedClaim[]): ExtractedClaim[] {
