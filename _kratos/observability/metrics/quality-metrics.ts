...
export class QualityMetrics {
  ...
  calculate(): QualityReport {
    ...
    const storyResult = db.exec(
      `SELECT DISTINCT story_key FROM trajectories WHERE story_key IS NOT NULL`
    );
    const storyKeys: string[] = storyResult.length
      ? storyResult[0].values.map((r: unknown[]) => r[0] as string)
      : [];
    ...
    // Per-gate stats
    const gateStats: Record<string, { total: number; passed: number }> = {};
    ...
    for (const storyKey of storyKeys) {
      const reviewResult = db.exec(
        `SELECT workflow, score FROM trajectories
         WHERE story_key = ? AND workflow LIKE '%review%'
         ORDER BY created_at ASC`,
        [storyKey]
      );
      if (!reviewResult.length) continue;

      stories_reviewed++;
      let allPassedFirst = true;

      // Track first attempt per gate per story
      const firstAttempt: Record<string, boolean> = {};

      for (const row of reviewResult[0].values) {
        const workflow = row[0] as string;
        const score = row[1] as number | null;
    ...
    const gate_results: ReviewGateResult[] = REVIEW_GATES.map(gate => ({
      gate,
  ...
  formatReport(report: QualityReport): string {
    ...
    // Header
    lines.push(
      padRight('Gate', 25) + padRight('Total', 8) +
