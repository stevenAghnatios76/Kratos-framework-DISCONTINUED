...
export class AgentMetrics {
  ...
  calculate(): AgentReport {
    ...
    const agentResult = db.exec(
      'SELECT DISTINCT agent_id FROM trajectories ORDER BY agent_id'
    );
    const agentIds: string[] = agentResult.length
      ? agentResult[0].values.map((r: unknown[]) => r[0] as string)
      : [];

    // Also check memory_entries for agents without trajectories
    const memAgentResult = db.exec(
      'SELECT DISTINCT agent_id FROM memory_entries ORDER BY agent_id'
    );
    ...
    for (const agent_id of agentIds) {
      ...
      const trajResult = db.exec(
        `SELECT COUNT(*), COALESCE(AVG(score), 0),
         ...
         FROM trajectories WHERE agent_id = ?`,
        [agent_id]
      );
      const total_workflows = (trajResult[0]?.values[0]?.[0] as number) || 0;
      const avg_score = (trajResult[0]?.values[0]?.[1] as number) || 0;
      const passed = (trajResult[0]?.values[0]?.[2] as number) || 0;
      const failed = (trajResult[0]?.values[0]?.[3] as number) || 0;

      // Patterns learned
      const patResult = db.exec(
        `SELECT COUNT(*) FROM memory_entries WHERE agent_id = ? AND partition = 'patterns' AND status = 'active'`,
        [agent_id]
      );
      const patterns_learned = (patResult[0]?.values[0]?.[0] as number) || 0;

      // Anti-patterns
      const antiResult = db.exec(
        `SELECT COUNT(*) FROM memory_entries WHERE agent_id = ? AND partition = 'anti-patterns' AND status = 'active'`,
        [agent_id]
      );
      const anti_patterns = (antiResult[0]?.values[0]?.[0] as number) || 0;

      // Cost from metrics table
      const costResult = db.exec(
        `SELECT COALESCE(SUM(value), 0) FROM metrics
         WHERE metric_type = 'cost' AND json_extract(dimensions, '$.agent_id') = ?`,
        [agent_id]
      );
      const total_cost = (costResult[0]?.values[0]?.[0] as number) || 0;
      ...
      const total_tokens = (tokenResult[0]?.values[0]?.[0] as number) || 0;
  ...
  formatReport(report: AgentReport): string {
    ...
    // Header
    lines.push(
      padRight('Agent', 20) + padRight('Workflows', 12) + padRight('Pass Rate', 12) +
