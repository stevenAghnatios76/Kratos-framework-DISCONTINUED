...
interface CachedContext {
  ...
  skill_sections: { skill: string; section: string; lines: number }[];
  total_lines: number;
...
export class ContextCache {
  private kratosRoot: string;
  private cacheDir: string;
  private skillIndex: SkillIndex;
  private budgetMax: number;

  constructor(kratosRoot: string, budgetMax: number = 40000) {
    ...
    this.skillIndex = new SkillIndex(skillsDir, skillsCacheDir);
  }
  ...
  buildAgentContext(agentId: string, personaPath: string): CachedContext | null {
    ...
    const skillSections: CachedContext['skill_sections'] = [];
   ...
   */
  checkBudget(agentId: string): { fits: boolean; tokens: number; budget: number; overage: number } {
    const cachePath = path.join(this.cacheDir, 'agents', `${agentId}.json`);
    let tokens = 0;
    if (fs.existsSync(cachePath)) {
      const cached = JSON.parse(fs.readFileSync(cachePath, 'utf-8')) as CachedContext;
  ...
  private extractSkillReferences(content: string): { skill: string; sections?: string[] }[] {
    const refs: { skill: string; sections?: string[] }[] = [];
  ...
  private estimateFullFileTokens(): number {
    let totalLines = 0;
    const dirs = [
      path.join(this.kratosRoot, 'dev', 'agents'),
  ...
  private saveCacheManifest(contexts: CachedContext[]): void {
  ...
  private loadCacheManifest(): CachedContext[] {
    const manifestPath = path.join(this.cacheDir, 'manifest.json');
